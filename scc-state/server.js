  ...
  for await (let [req, res] of httpServer.listener('request')) {
    ...
    if (req.url === '/login') {
      req.on('data', data => {
        ...
        if (user) {
          let userData = _.omit(user, ['password']);
          res.writeHead(200, {
...
let sccWorkerLeaveCluster = function (socket, req) {
  ...
  logInfo(`The scc-worker instance ${socket.instanceId} at address ${socket.instanceIp}:${socket.instancePort} left the cluster on socket ${socket.id}`);
};
  ...
  for await (let {
    ...
    (async () => {
      for await (let req of socket.procedure('sccWorkerJoinCluster')) {
        ...
        if (!serverReady) {
          logWarning(`The scc-worker instance ${data.instanceId} at address ${socket.instanceIp}:${socket.instancePort} on socket ${socketId} was not allowed to join the cluster because the server is waiting for initial brokers`);
          req.error(
        ...
        logInfo(`The scc-worker instance ${data.instanceId} at address ${socket.instanceIp}:${socket.instancePort} joined the cluster on socket ${socketId}`);
      }
    })();
